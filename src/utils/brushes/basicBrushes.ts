...
const circleBrush = {
  ...
  preview: (ctx: BrushContext) => ({ x: ctx.x - ctx.brush.size, y: ctx.y - ctx.brush.size, size: ctx.brush.size * 2 + 1 }),
};
...
const sprayBrush = {
  ...
  preview: (ctx: BrushContext) => ({ x: ctx.x, y: ctx.y, size: (ctx.brush.sprayRadius ?? Math.floor(ctx.brush.size / 2)) * 2 }),
};
