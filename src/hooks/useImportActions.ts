...
import {
  importAnimationJSON, importAlembicABC,
} from '../utils/animationExporter';
import {
  isOldDemoFile, oldDemoToProject, deserializeProject,
...
export function useImportActions(
  ...
  const handleImportAnimation = useCallback(async (format: 'json' | 'abc') => {
    try {
      ...
      if (format === 'json') {
        ...
        if (!result) { onToast('Invalid animation JSON file', 'error'); setLoadingState({ isLoading: false }); return; }
        ...
        if (!result) { onToast('Invalid Alembic ABC file', 'error'); setLoadingState({ isLoading: false }); return; }
        setCanvasState(result.canvasState);
        setTimeline(result.timeline);
        setLoadingState({ isLoading: false });
