...
const Canvas2D: React.FC<Canvas2DProps> = ({
  ...
  const pixelSize =
    Math.min(containerSize.width / canvasState.width, containerSize.height / canvasState.height, 20) * scale;

  // Helper: build or update a single layer's OffscreenCanvas cache
  const updateLayerCache = useCallback(
    (layerIdx: number, useDirtyRegion?: boolean) => {
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Draw brush outline
    if (hoverPixel.current) {
      const bx = hoverPixel.current.x;
      const by = hoverPixel.current.y;
      ...
      const rectX = (bx - half) * pixelSize;
      const rectY = (by - half) * pixelSize;
  ...
  const handleWheel = useCallback(
    (e: WheelEvent) => {
      ...
      setScale((prev) => {
        ...
        const newPixelSize =
          Math.min(containerSize.width / canvasState.width, containerSize.height / canvasState.height, 20) * newScale;
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-2xl relative">
      <div
        ...
        className="w-full h-64 md:h-96 flex items-center justify-center overflow-hidden relative"
      >
        <canvas
          ref={canvasRef}
      ...
      <div ref={localPreviewRef} className="absolute bottom-2 right-2 w-20 h-20 border-2 border-gray-600 bg-gray-800" />
    </div>
  );
};
