...
const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ activeTab, onTabChange, onMenuPress }) => {
  return (
    <nav className="mobile-nav bg-panel border-t border-border flex-shrink-0" role="navigation" aria-label="Main navigation">
        ...
        {navItems.map(({ tab, label, icon }) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 transition-all duration-200 touch-target-min relative ${
              activeTab === tab
                ? 'text-accent'
                : 'text-text-dim active:text-text'
            }`}
            aria-label={label}
          ...
          >
            ...
            {activeTab === tab && (
              <span className="absolute inset-x-0 top-0 h-0.5 bg-accent rounded-full mx-4" />
            )}
            <span className={`transition-transform duration-200 ${activeTab === tab ? 'scale-110' : ''}`}>
              {icon}
            </span>
