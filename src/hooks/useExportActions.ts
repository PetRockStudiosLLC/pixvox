...
import {
  exportAnimationJSON,
  exportAlembicABC, exportFrameSequenceGLTF,
} from '../utils/animationExporter';
...
export function useExportActions(
  ...
  const handleExportGLTF = useCallback(() => {
    try {
      ...
      if (!content) { onToast('No voxels to export!', 'warning'); return; }
  ...
  const handleExportAnimationJSON = useCallback(() => {
    try {
      const { content, filename, mimeType } = exportAnimationJSON(canvasState, timeline);
      if (!content) { onToast('No frames to export!', 'warning'); return; }
      downloadFile(content, filename, mimeType);
      onToast(`Exported ${filename}`, 'success');
    } catch (error) {
  ...
  const handleExportAlembicABC = useCallback(async (compress: 'delta' | 'snapshot') => {
    try {
      ...
      if (!blob.size) { onToast('No frames to export!', 'warning'); setLoadingState({ isLoading: false }); return; }
  ...
  return {
    handleExportGLTF, handleExportAnimationJSON, handleExportAlembicABC,
    handleExportFrameSequence,
    handleExportPNG, handleExportSpriteSheet, handleExportGIF,
  };
}
